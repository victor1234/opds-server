...
- Bound catalog pagination to prevent requests beyond the final page.
- Made catalog ordering deterministic.
- Omitted navigation links from empty catalog pages.
- Validated configuration during startup readiness checks.
- Handled invalid Calibre timestamps.
- Returned HTTP 503 when the Calibre database is unavailable.
- Honored the configured catalog prefix in OPDS feeds.

## [0.1.2] - 2026-07-19

### Security

- Prevented book download requests from accessing files outside the configured Calibre library.
...
- Added CI concurrency to cancel superseded builds.
